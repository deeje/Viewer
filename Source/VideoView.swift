...
class VideoView: UIView {
    ...
    weak var parentController: UIViewController? {
        didSet {
            if let parentController {
                parentController.addChild(self.playerController)
                self.addSubview(self.playerController.view)
                self.playerController.didMove(toParent: parentController)
                self.playerController.beginAppearanceTransition(true, animated: false)
            }
        }
    }
    ...
    fileprivate lazy var playerController: AVPlayerViewController = {
    ...
    private lazy var loadingIndicatorBackground: UIView = {
    ...
    override func layoutSubviews() {
        ...
        self.playerController.view.frame = playerLayerFrame
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change _: [NSKeyValueChangeKey: Any]?, context _: UnsafeMutableRawPointer?) {
        ...
        if let error = playerItem.error {
            ...
            if self.playerController.view.isHidden == false {
                player.play()
    ...
    func stopPlayingAndSeekSmoothlyToTime(duration: Double) {
        guard let timescale = self.playerController.player?.currentItem?.currentTime().timescale else { return }
    ...
    func actuallySeekToTime() {
        ...
        self.playerController.player?.seek(to: seekTimeInProgress, toleranceBefore: CMTime.zero, toleranceAfter: CMTime.zero) { _ in
...
extension VideoView {
    ...
    fileprivate func addPlayer(using viewable: Viewable, completion: @escaping () -> Void) {
        if let assetID = viewable.assetID {
            ...
            DispatchQueue.global(qos: .background).async {
                self.playerController.player = AVPlayer(url: streamingURL)
                self.playerController.view.isHidden = true
    ...
    fileprivate func removeWhilePlayingObservers() {
        if let playbackProgressTimeObserver = self.playbackProgressTimeObserver {
            self.playerController.player?.removeTimeObserver(playbackProgressTimeObserver)
            self.playbackProgressTimeObserver = nil
        }
